...
func (c *RedisClient) GetProviderV1(ctx context.Context, provider string) (v1Response.GetProviderData, error) {
	cmd := c.client.Get(ctx, fmt.Sprintf("v1:provider:%s", provider))

	cmdb, err := cmd.Bytes()
	if err != nil {
		return v1Response.GetProviderData{}, err
	}
...
func (c *RedisClient) SetProviderV1(ctx context.Context, provider string, p v1Response.GetProviderData) error {
	var b bytes.Buffer
