...
// @Router			/api/v1/chapters/{provider_slug}/{series_slug}/{chapter_slug}/_bc [get]
func (h *Handler) GetChapterBreadcrumbs(c echo.Context) error {
	span := sentry.StartSpan(c.Request().Context(), "v1.GetChapterBreadcrumbs")
	...
	c.Logger().Debugj(map[string]interface{}{
		"_source":       "v1.GetChapterBreadcrumbs",
		"provider_slug": providerSlug,
		"series_slug":   seriesSlug,
		"chapter_slug":  chapterSlug,
	})

	cache, err := h.redis.GetChapterBreadcrumbsV1(c.Request().Context(), providerSlug, seriesSlug, chapterSlug)
	if err == nil {
	...
	result := v1Response.NewChapterBreadcrumbs(provider.Name, series.Title, chapter.ShortTitle)

	err = h.redis.SetChapterBreadcrumbsV1(c.Request().Context(), providerSlug, seriesSlug, chapterSlug, result)
	if err != nil {
		middlewares.SentryHandleInternalError(c, span, err, "redis.SetChapterBreadcrumbsV1")
	}
